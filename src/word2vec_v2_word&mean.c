//this is a new version of word2vec 

//this version use word and its prime meaning separately
#include <stdio.h>  
#include <stdlib.h>  
#include <string.h>  
#include <math.h>  
...
const int wordmeans_hash_size = 30000000;  // Maximum 30 * 0.7 = 21M words in the wordmeansulary  
/////
...
struct wordmeans_wordmean{
  ...
  int *point;
  char *mean;//can also be used as word
  char *code,codelen;//
};
...
//the energy table of each wordmeans,used in  negative sampling
//实际上是统计单词的频率
...
int AddWordTowordmeans(char *word)  
{ 
  unsigned int hash, length = strlen(word) + 1;  
  if (length > MAX_STRING)  
      length = MAX_STRING;  
  wordmeans[wordmeans_size].mean= (char *)calloc(length, sizeof(char));  
  strcpy(wordmeans[wordmeans_size].mean, word); 
  wordmeans_size++; 
  ...
  wordmeans_hash[hash] = wordmeans_size - 1;
  //由词的hash值找到她所在词汇表的排序位置  
  return wordmeans_size - 1;  
}


//add the word and its means 
  
// Returns hash value of a word返回一个词的hash值，一个词跟hash值一一对应（可能冲突）  

// Returns position of a word in the wordmeansulary; if the word is not found, returns -1  
// 返回一个词在词汇表中的位置，如果不存在则返回-1  

int SearchWordMean_Hownet(char *word){
...
int SearchWordMean(char *wordmean){
  ...
  {  
    ...
    strcpy(wordmean2,wordmeans[wordmeans_hash[hash]].mean);
...
// Reads a word and returns its index in the wordmeansulary  
// 从文件流中读取一个词，并返回这个词在词汇表中的位置  
// ?

// Adds a word to the wordmeansulary 将一个词添加到一个词汇中  
...
{  
  ...
  char *wordmean=(char *)calloc(strlen(mean)+1,sizeof(char));
...
// Sorts the wordmeansulary by frequency using word counts  
// 根据词频排序  
...
{  
  ...
  int a, size;  
  unsigned int hash; 
  char mean[2*MAX_STRING];
  // Sort the wordmeansulary and keep </s> at the first position 
  qsort(&wordmeans[1], wordmeans_size - 1, sizeof(struct wordmeans_wordmean), WordMeanCompare); 
  for (a = 0; a < wordmeans_hash_size; a++)  //因为wordmeans已经重新排序了，所以要重新生成HASH表
      wordmeans_hash[a] = -1;  
  size = wordmeans_size;  
  for (a = 0; a < size; a++)  
  {  
    // Words occuring less than min_count times will be discarded from the wordmeans  
    //出现太少的词直接丢弃  
    ...
    {  
      ...
      // 重新计算hash查找。wordmeans_hash是由hash值找到该词所在位置 

      strcpy(mean,wordmeans[a].mean);
      hash=GetWordMeanHash(mean); 
...
// Reduces the wordmeansulary by removing infrequent tokens  
// 再次移除词频过小的词，缩减词汇表  
///////////
...
{  
  ...
  for (a = 0; a < wordmeans_size; a++) {  
    ...
    strcpy(mean,wordmeans[wordmeans_hash[hash]].mean);
    hash = GetWordMeanHash(mean); 
    while (wordmeans_hash[hash] != -1) hash = (hash + 1) % wordmeans_hash_size;  
    wordmeans_hash[hash] = a;  
  }  
...
void ImportHowNetMeans( char *file){
  ...
  while(!feof(fin)){
  	  ReadWord(word,fin);
      Readline(means,fin);
      int i = SearchWordMean_Hownet(word);//返回该词在词汇表中的位置  
     if (i == -1)//该词之前不存在  
      {  
...
void CreateBinaryTree_WordMean() {  
  ...
  // Now assign binary code to each wordmeansulary word  
 //分配编码，非常有用
  for (a = 0; a < wordmeans_size; a++)  
  {  
    b = a;  
...
{ 
  ...
  {  
    ...
    i = SearchWordMean(word);//返回该词在词汇表中的位置  
    hownet_pos=SearchWordMean_Hownet(word);
    ...
    {  
    	...
    	if(i==-1){
    		a=AddWordTowordmeans(word);//add word to wordmeans actually
    		wordmeans[a].cn=1;
    	}
    	else{
    		wordmeans[i].cn++;
    	...
    	if (wordmeans_size > wordmeans_hash_size * 0.7){//如果词汇表太庞大，就缩减 //because the wordmeans and wordmeans have similary process of increasement    
        	ReduceWordMeans();
     	} 
  	}
  }  
  printf("%lld,%lld\n",train_words,wordmeans_size);
   printf("%lld\n",wordmeans_size); 
  //Sortwordmeans();
  SortWordMeans();//根据词频排序词汇  
  if (debug_mode > 0)  
  {   
...
void SaveWordMeans() {  
  ...
  for (i = 0; i < wordmeans_size; i++) {
  	fprintf(fo, "%s %lld\n",wordmeans[i].mean,wordmeans[i].cn);  
  }
  fclose(fo);  
}  
//////////////////////// 
...
{  
  ...
  if (fin == NULL)  
  {  
    printf("wordmeansulary file not found\n");  
    exit(1);  
  }  
  for (a = 0; a < wordmeans_hash_size; a++)  
      wordmeans_hash[a] = -1;  
  ...
  {  //从fin进入一个词到word中  
    ...
    ////////
    int hash = GetWordMeanHash(mean);  
    while (wordmeans_hash[hash] != -1){//如果hash值冲突了  
      hash = (hash + 1) % wordmeans_hash_size;
    }
    ///////
    a = AddWordMeanToWordMeans(mean);//把该词添加到词汇中，并返回该词的位置  
...
{  
  ...
  {  
    ...
    {  
      while (1)  
      {  
        char wordstr[2*MAX_STRING];  
  		ReadWord(wordstr, fi);  
  		if (feof(fi)) break; 
 		  word=SearchWordMean(wordstr);//从文件流中读取一个词，并返回这个词在词汇表and in hownet中的位置  
        if (word == -1) continue;  
        word_count++;  
        //if (word == 0) break;  ?
        ...
        {  
          ...
          real ran = (sqrt(wordmeans[word].cn / (sample * train_words)) + 1) * (sample * train_words) / wordmeans[word].cn;  
  ...
  //基于伪随机数的算法 http://www.cnblogs.com/xkfz007/archive/2012/08/25/2656893.html
    ...
    if (cbow) {  //train the cbow architecture
      ...
      if (hs) for (d = 0; d < wordmeans[word].codelen; d++) {
        f = 0;
        l2 = wordmeans[word].point[d] * layer1_size;
        ...
        g = (1 - wordmeans[word].code[d] - f) * alpha;
    ...
    hownet_pos=SearchWordMean_Hownet(wordmeans[word].mean);
    if(hownet_pos==-1){
      sentence_position++;
      continue;
    }
    ...
    {	
      	...
      	if(means[res_pos]==' ' ) 
      	{
      		mean[des_pos]='\0';
      		while(1){
      			if(mean[temppos]=='\0')break;
      			if(mean[temppos]==127)mean[temppos]='\0'; 
      			temppos++;
      		}
      		wordmean_pos=SearchWordMean(mean);
      		if (cbow) 
      		{  //train the cbow architecture  w
      			// in -> hidden
...
//初始化词向量，一共有wordmeans_size个单词，每个单词词向量维度是layer1_size
  printf("wordmeans_size:%lld\n",wordmeans_size );
  for (b = 0; b < layer1_size; b++)  
      for (a = 0; a < wordmeans_size; a++){  
...
//value_ptr：退出线程的返回值。
  ...
  {  
    ...
    int *cl = (int *)calloc(wordmeans_size, sizeof(int));//词到类别的映射  
    real closev, x;  
...
int main(int argc, char **argv) {  
  ...
  if (argc == 1) {  
    ...
    //保存词汇到这个文件  
    printf("\t-save-wordmeans <file>\n");  
    printf("\t\tThe wordmeansulary will be saved to <file>\n");  
  
    //词汇从该文件读取，而不是由训练数据重组  
    printf("\t-read-wordmeans <file>\n");  
    printf("\t\tThe wordmeansulary will be read from <file>, not constructed from the training data\n");  
  ...
  if ((i = ArgPos((char *)"-save-wordmeans", argc, argv)) > 0) strcpy(save_wordmeans_file, argv[i + 1]);  
  if ((i = ArgPos((char *)"-read-wordmeans", argc, argv)) > 0) strcpy(read_wordmeans_file, argv[i + 1]);  
